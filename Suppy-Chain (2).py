...
def viewUser(user_index, user_type):
	#view all transactions of the user
	#user type = 0 for manufacturers
	#user type = 1 for stakeholder(other_users)
	supply_blockchain_temp = supply_blockchain[1:]
	if(user_type == 1):
		#stakeholder
		user_key = other_users_list[user_index]
	if(user_type == 0):
		user_key = manufacturers_list[user_index]

	user_puk = user_key.publickey().exportKey("PEM").decode('utf-8')
	for block in supply_blockchain_temp:
		for transaction in block.supply_data:
			if((transaction.receiver_puk == user_puk) or (str(transaction.supplier_puk.exportKey("PEM").decode('utf-8')) == user_puk)):
				print('\n-------------------------------------------------------------------------')
				print(transaction.supplier_puk.exportKey("PEM").decode('utf-8'))
				print(transaction.receiver_puk)
				print(transaction.item_id)
...
def verify_transaction(self):
	...
	try:
		...
		flag = False
	return verify(gen_public_sig(self.item_id, message)) and flag
	# return True
...
def convert_message_to_int(M):
	return int(sha256(M.encode('utf-8')).hexdigest(), 16)
...
while(1):
	...
	if(choice == 1):
		...
		user_index = int(input('Enter the user: '))
		user_type = int(input('Enter the user type(0 for manufacturers, 1 for others): '))
		viewUser(user_index,user_type)
	elif(choice == 8):
		break
	else:
